...
return (
    <div> 
    {/*Because the question is a single item array, I used .map to pull the item out as a string and display it. Could be more efficient  */}
    {question.length > 0 ? (
            question.map((item) => (
    ...
    {/* Uses .map to display the choices with radio buttons, assigning the value to the ID and quantity of votes */}
            {choices.length > 0 ? (choices.map((item) => (
                    <li style={{color: "black", fontSize: "16px"}}> 
                        <input 
                            type="radio"
                            //Asigns as a string, when I would prefer to have an array. Adds challenges later on.
                            value={[item.id, item.votes]} 
                            name="choice"
                            //Updates state of radio variable with the choice ID and quantity of votes each time
        ...
        <br></br>
        {/* Button initates the updateVote function on click. See updateVote above */}
